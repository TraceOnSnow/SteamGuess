...
for (const path of ['dist/index.html', 'public/games_demo.json', 'public/labeling_catalog.json', 'server/index.js']) {
...
if (existsSync('public/games_demo.json')) {
  ...
  games.length >= 1900 ? pass(`playable catalog has ${games.length} games`) : fail(`playable catalog has only ${games.length} games`);
  invalid === 0 ? pass('playable catalog shape and AppIDs') : fail(`${invalid} catalog records are invalid`);
...
if (process.env.VITE_LABELER_ENABLED === 'true') warn('VITE_LABELER_ENABLED=true exposes the internal labeler in this build');
else pass('internal labeler disabled by default');
...
console.log(`READY checks=${7 + (existsSync('public/games_demo.json') ? 4 : 0)} warnings=${warnings.length}`);
