...
export default function MultiplayerPage() {
  ...
  useEffect(() => {
    const socket = io({ path: '/socket.io', transports: ['websocket', 'polling'] }); socketRef.current = socket;
    ...
    if (resumeIdentity) socket.on('connect', () => { void emit<{ room: RoomSnapshot }>(socket, 'room:resume', resumeIdentity).then(response => { if (response.ok) setRoom(response.data.room); else sessionStorage.removeItem(SESSION_KEY); }); });
  ...
  const currentResults = useMemo(() => results.filter(result => result.roundId === room?.activeRound?.id), [results, room?.activeRound?.id]);
  ...
  async function leave() { if (room) await send('room:leave', { roomId: room.id }); sessionStorage.removeItem(SESSION_KEY); window.location.assign('/'); }
  ...
  return (
    <main className="mp-shell">
      ...
      {room.status === 'countdown' ? <section className="mp-card mp-countdown">
        ...
        <div className="mp-actions"><button className={`btn ${me?.ready ? 'btn-quiet' : 'btn-primary'}`} onClick={() => void ready()}>{me?.ready ? '取消准备' : '准备'}</button>{room.hostPlayerId === identity?.playerId && <button className="btn btn-primary" disabled={room.players.length < 2 || room.players.some(player => !player.ready)} onClick={() => void start()}>开始比赛</button>}</div>
      </section> : <>
        <section className="mp-playbar"><div><span>剩余时间</span><strong>{secondsLeft}s</strong></div>{room.status === 'playing' && <><SearchBox games={games} excludedAppIds={excluded} onSelectGame={game => void guess(game)} isDisabled={!me?.connected || me?.finished} /><button className="btn btn-quiet" onClick={() => void surrender()}>投降</button></>}</section>
        ...
        {room.status === 'finished' && <section className="mp-card"><h2>{room.match?.winnerPlayerId === identity?.playerId ? '你赢得了比赛' : '比赛结束'}</h2><button className="btn btn-primary" onClick={() => void rematch()}>{me?.rematch ? '等待其他玩家确认…' : '再来一局'}</button></section>}
